...
LOCAL_REF="skills/creart-prompt/references"
...
`skills/creart-prompt/SKILL.md` 是 Advisor-only 技能定义。从 upstream `skills/gpt-image-2/SKILL.md` 提取，按以下规则适配。
...
- **输出目录** — 只保留 prompt 目录（`skills/creart-prompt/prompt/`）
- **命名规则** — 只保留 prompt 命名
...
**路径替换**：upstream 中所有 `garden-gpt-image-2/prompt/` 替换为 `skills/creart-prompt/prompt/`。
...
LOCAL_COUNT=$(find skills/creart-prompt/references -name '*.md' | wc -l | tr -d ' ')
...
advisor-only 模式不携带任何脚本。逐一检查 `skills/creart-prompt/references/` 下的分类目录，确认其中模板不依赖不存在的脚本或功能：
...
grep -rl 'scripts/' skills/creart-prompt/references/ 2>/dev/null | \
  sed 's|skills/creart-prompt/references/||;s|/.*||' | sort -u | \
  while read -r dir; do
...
2. 删除 `skills/creart-prompt/references/` 下对应目录
...
```bash
SKILL="skills/creart-prompt/SKILL.md"
